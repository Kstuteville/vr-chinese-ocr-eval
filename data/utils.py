...
def _save_load_data_splits_to_npy(
    *,
    output_dir,
    X_train,
    X_test,
    ...
    """Save load_data outputs to NPY files."""
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    # Save only canonical filenames used by the notebook workflow.
    np.save(output_dir / "X_train.npy", X_train, allow_pickle=True)
    np.save(output_dir / "y_train.npy", y_train, allow_pickle=True)
    np.save(output_dir / "X_test.npy", X_test, allow_pickle=True)
    np.save(output_dir / "y_test.npy", y_test, allow_pickle=True)
    np.save(output_dir / "pert_types.npy", perturbation_type_test, allow_pickle=True)
...
def load_data(
    ...
    perturb_bucket_count=None,
    save_npy=False,
    npy_output_dir="data/cache",
    # Retained for backward compatibility; not used when saving canonical filenames.
    npy_file_prefix=None,
):
    """Load CASIA, build disjoint clean/perturb pools, perturb, and split."""
    selected_buckets = select_perturbation_buckets(
        include_combinations=include_combinations,
    ...
    if save_npy:
        _save_load_data_splits_to_npy(
            output_dir=npy_output_dir,
            X_train=X_train,
            X_test=X_test,
            y_train=y_train,
            y_test=y_test,
            perturbation_type_train=perturbation_type_train,
            perturbation_type_test=perturbation_type_test,
        )
